...
ggplot(data = sample)+geom_point(mapping = aes(x = carat, y = price))
# Draw the same plot as above, but color each of the points based on their clarity.
ggplot(data = sample)+geom_point(mapping = aes(x = carat, y = price, color = clarity))
...
ggplot(data = sample)+geom_point(mapping = aes(x = carat, y = price, color = 'blue'))


# Draw a scatter plot for `diamonds.sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.
ggplot(data = sample)+geom_point(mapping = aes(x = carat, y = price, shape = cut))


# Draw a scatter plot for `diamonds.sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
ggplot(data = sample)+geom_point(mapping = aes(x = carat, y = price, size = price))

# Try coloring the above plot based on the diamond's price!
ggplot(data = sample)+geom_point(mapping = aes(x = carat, y = price, size = price, color = price))
...
ggplot(data = sample)+geom_line(mapping = aes(x = carat, y = price, color = carat))


# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = sample)+geom_smooth(mapping = aes(x = carat, y = price, color = carat))


# Draw a plot with bar geometry (a bar chart), mapping the diamond's `cut` to the x-axis
ggplot(data = sample)+geom_bar(mapping = aes(x = cut))


# Add an aesthetic property that will _fill_ each bar geometry based on the `clarity` of the diamonds
# What kind of chart do you get?
ggplot(data = sample)+geom_bar(mapping = aes(x = cut, fill = clarity))


# Draw a histogram of diamond prices.
# Try mapping each bar based on clarity as well!
ggplot(data = sample)+geom_histogram(mapping = aes(x = price, fill = clarity))

# (For a more traditional "bell-curve", make a histogram of diamond `depths`)
ggplot(data = sample)+geom_histogram(mapping = aes(x = depths, fill = clarity))
